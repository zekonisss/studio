  ...
  return (
    <Card className="shadow-sm border-orange-500/20 h-fit">
      <CardHeader className="pb-3 border-b border-orange-100 dark:border-orange-500/10">
        ...
        <p className="text-xs text-muted-foreground">{t('activityFeed.description')}</p>
      </CardHeader>
      
      <CardContent className="pt-4">
        <div className="space-y-4">
          {loading ? (
            <div className="text-center py-4 text-xs text-muted-foreground">{t('activityFeed.loading')}</div>
          ) : recentLogs.length > 0 ? (
            recentLogs.map((log) => (
              <div key={log.id} className="flex items-start gap-3 pb-3 border-b border-slate-100 dark:border-slate-800 last:border-0 last:pb-0">
                ...
                <div className="grid gap-0.5 w-full">
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <span>{t('activityFeed.check')}</span>
            ...
            <div className="text-center py-4 text-xs text-muted-foreground">{t('search.noResults.title')}</div>
          )}
        </div>
        ...
        <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center gap-2 text-[10px] text-muted-foreground">
          ...
          <span>{t('activityFeed.anonymized')}</span>
        </div>
      </CardContent>
    </Card>
  );
