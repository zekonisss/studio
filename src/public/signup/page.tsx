...
export default function SignupPage() {
  ...
  const onSubmit = async (values: SignupFormValuesExtended) => {
    ...
    try {
      ...
      router.push('/public/activation-pending');
    } catch (error: any) {
  ...
  return (
    <Card className="w-full max-w-lg">
      <CardHeader className="items-center text-center">
        <Link href="/public/login" className="flex items-center gap-2 text-lg font-semibold text-primary cursor-pointer mb-4">
            <UserSearch className="h-12 w-12" />
            <span className="text-4xl font-bold">DriverCheck</span>
        </Link>
        <CardTitle className="text-2xl !mt-6">{t('signup.title')}</CardTitle>
                ...
                <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                  ...
                  <div className="space-y-1 leading-none">
                    <FormLabel>
                      ...
                      <Link href="/public/terms" target="_blank" className="underline hover:text-primary">
