...
export default function OpsCenterPage() {
  ...
  const [analysisData, setAnalysisData] = useState<boolean>(false);
  const [isProcessing, setIsProcessing] = useState(false);

  const handleUpload = (file: File) => {
    setIsProcessing(true);
    // Simulate reading the .ddd file and processing
    setTimeout(() => {
  ...
  const renderContent = () => {
    if (isProcessing) {
      return (
        <div className="flex flex-col items-center justify-center h-96 gap-4 text-muted-foreground">
          ...
          <p className="text-lg font-medium">Analizuojamas .DDD failas...</p>
        </div>
      );
    }
    ...
    // Default view: just the upload zone
    return <OpsUploadZone onFileSelected={handleUpload} />;
  };
