...
export function ReportCard({ report, onViewDetails, onDelete, isDeleted = false }: ReportCardProps) {
  ...
  const isDestructive = DESTRUCTIVE_REPORT_MAIN_CATEGORIES.some(
    (cat) => cat === report.category
  );

  const formattedDate = (isDeleted && report.deletedAt) 
    ? new Date(report.deletedAt).toLocaleDateString(locale)
    : (report.createdAt ? new Date(report.createdAt).toLocaleDateString(locale) : '');
            ...
            <CardDescription className="flex items-center gap-4 text-xs pt-1">
              {report.nationality && (
                <span className="flex items-center gap-1.5">
                  <Globe className="w-3 h-3" /> {t(`countries.${report.nationality}`)}
                </span>
              )}
              {report.birthYear && (
                <span className="flex items-center gap-1.5">
                  <User className="w-3 h-3" /> {report.birthYear}
                </span>
              )}
            </CardDescription>
          </div>
