  ...
  useEffect(() => {
    if (!loading && user) {
        if (user.paymentStatus === 'active') {
            ...
            router.replace('/activation-pending'); // Pataisyta: buvo /public/...
        }
    }
  }, [user, loading, router]);
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader className="items-center text-center">
        {/* 1. PATAISYTA NUORODA ČIA */}
        <Link href="/login" className="flex items-center gap-2 text-lg font-semibold text-primary cursor-pointer mb-4">
      ...
      <CardContent>
        ...
        <div className="mt-4 text-center text-sm">
          {t('login.noAccount')}{" "}
          {/* 2. PATAISYTA NUORODA ČIA (Svarbiausia) */}
          <Link href="/signup" className="underline">
            {t('login.signupLink')}
          </Link>
        </div>
        <div className="mt-2 text-center text-sm">
           {/* 3. PATAISYTA NUORODA ČIA */}
