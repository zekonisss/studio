...
export default function ProtectedLayout({
  ...
  useEffect(() => {
    ...
    // If loading is finished, check for user and payment status
    if (!user || user.paymentStatus !== 'active') {
      router.replace('/login');
    }
  }, [user, loading, router]);
  
  // Show a full-screen loader while we are determining the auth state or if the user is invalid
  ...
  // Only when loading is complete AND we have a valid, active user, render the full layout.
  return (
