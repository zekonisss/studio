...
export default function TeamPage() {
  ...
  const handleInvite = async () => {
    ...
    try {
        ...
        toast({ variant: "destructive", title: "Klaida", description: "Serverio klaida." });
    } finally {
        setIsInviting(false);
    }
  };
  ...
  const handleRemove = async (memberId: string) => {
    ...
    try {
        ...
        if (res.success) {
            ...
            toast({ variant: "destructive", title: "Klaida", description: res.error });
        }
    } catch (e) {
        console.error(e);
    }
  ...
  const getRoleBadge = (role: TeamMember['role']) => {
      switch(role) {
          case 'owner':
              return <Badge variant="default" className="bg-amber-500 hover:bg-amber-600"><Crown className="mr-1 h-3 w-3" /> Savininkas</Badge>;
          case 'admin':
              return <Badge variant="secondary"><Shield className="mr-1 h-3 w-3" /> Administratorius</Badge>;
          case 'member':
              return <Badge variant="outline"><UserIcon className="mr-1 h-3 w-3" /> Narys</Badge>;
          default:
              return null;
      }
  }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p className="text-muted-foreground">
                Įmonė: <span className="font-semibold text-foreground">{companyInfo.name}</span>
            </p>
        </div>
