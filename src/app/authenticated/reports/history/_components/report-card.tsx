...
import { Eye, Trash2, Globe, User, Calendar } from "lucide-react";
...
export function ReportCard({ report, onViewDetails, onDelete, isDeleted = false }: ReportCardProps) {
  ...
  const displayDate = report.createdAt 
    ? new Date(isDeleted && report.deletedAt ? report.deletedAt : report.createdAt).toLocaleDateString(t('common.localeForDate'))
    : "";
    ...
    <Card className={`flex flex-col ${isDeleted ? 'opacity-60 bg-muted/50' : ''} ${isDestructive && !isDeleted ? 'border-destructive/30 hover:border-destructive/50' : 'hover:bg-accent/10'}`}>
      <CardHeader>
        <div className="flex justify-between items-start gap-2">
          ...
          <Badge variant={isDestructive && !isDeleted ? 'destructive' : 'secondary'}>
