...
export default function ReportsImportPage() {
    ...
    try {
        ...
        const findHeader = (possibleNames: string[]): string | undefined => {
            ...
            for (const name of possibleNames) {
                const found = headerRow.find(h => h.trim().toLowerCase() === name);
                if (found) return found;
            }
            return undefined;
        };

        const fullNameHeader = findHeader(['title', 'vardas pavardė', 'full name', 'name']);
        const commentHeader = findHeader(['comment', 'comments', 'komentaras']);
        const companyHeader = findHeader(['company', 'įmonė']);
        const dateHeader = findHeader(['date', 'data']);

        if (!fullNameHeader || !commentHeader) {
            toast({ variant: "destructive", title: t('reports.import.toast.missingHeaders.title'), description: `Įsitikinkite, kad faile yra "Title" ir "Comment" stulpeliai.` });
            setIsParsing(false);
            return;
        }

        const clientRecords: ClientParsedRecord[] = jsonData.map((row, index) => ({
            id: index + 2,
            ...
            createdAt: dateHeader && row[dateHeader] ? new Date(row[dateHeader]).toISOString() : new Date().toISOString(),
  ...
  const processRecordsWithAI = async (recordsToProcess: ClientParsedRecord[]) => {
      ...
      for (const record of recordsToProcess) {
          ...
          if (dailyQuotaReached) {
              updateRecordStatus(record.id, { status: 'skipped_quota' as const });
              continue;
          }
          if (!record.comment) {
              updateRecordStatus(record.id, { status: 'error' as const, error: t('reports.import.error.noCommentForAi')});
              continue;
          }
  ...
  const handleImportAll = async () => {
    ...
    const recordsToImport = records.filter(r => ['completed', 'error', 'skipped_quota'].includes(r.status));
    if (recordsToImport.length === 0) return;
    
    setIsImporting(true);
    try {
        const cleanData = recordsToImport.map(r => ({
          ...r,
          status: 'active' as const
        }));

        const result = await importAllReports(cleanData, user.id, user.companyName);
  ...
  const handleCancel = () => {
    ...
    setRecords(prev => prev.map(r => r.status === 'processing' || r.status === 'pending' ? { ...r, status: 'error' as const, error: 'Cancelled by user' } : r));
  };
      ...
      <CardHeader>
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <CardDescription>
                {t('reports.import.description')} Privalomi stulpeliai: <strong>&quot;Title&quot;</strong> ir <strong>&quot;Comment&quot;</strong>.
            </CardDescription>
          </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {records.length > 0 && (
            <div className="space-y-4">
                <div className="flex justify-between items-center">
                    <h3 className="text-lg font-semibold">{t('reports.import.previewTitle')} ({records.length})</h3>
                     <Button onClick={handleImportAll} disabled={isImporting || isParsing || records.some(r => r.status === 'processing' || r.status === 'pending')}>
                        ...
                        {isImporting ? t('reports.import.button.importing') : t('reports.import.button.importAll', { count: records.filter(r=> ['completed', 'error', 'skipped_quota'].includes(r.status)).length })}
                    </Button>
                </div>
        ...
        {!file && !isParsing && records.length === 0 && (
             <div className="text-center py-16 px-6 border-2 border-dashed rounded-lg">
                ...
                <p className="mt-2 text-sm text-muted-foreground">Norėdami pradėti, įkelkite .xlsx formato failą.</p>
            </div>
        )}
      </CardContent>
    </Card>
